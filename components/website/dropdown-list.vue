...
<div class="p-absolute bg-white box-shadow animate__animated  animate__fadeIn animate__faster pt-2 pb-2" :style="styling" ref="container" @mouseleave="remove($event)" @mouseenter="keep()">
    ...
    <button @click="select($event, item)" :class="selected == item.title.replace(' ', '-') ? 'bg-primary text-white box-shadow': ''" class="btn btn-block btn-hov text-left p-2 pl-3 pr-3" v-for="(item, i) in list" :key="i" v-html="item.title" >
    </button>
</div>
</template>
